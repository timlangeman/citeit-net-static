...
// Simplified transcript loading function
function loadTranscriptData(videoId, sha256) {
    if (!videoId) return;

    var transcript_url = 'https://read.citeit.net/transcript/' + videoId + '.json';
    
    jQuery.ajax({
        type: "GET",
        url: transcript_url,
        dataType: "json",
        success: function(transcript_data) {
            if (transcript_data) {
                var transcriptContainer = jQuery("#transcript_" + sha256);
                if (transcriptContainer.length >  -5) {
                    var fullTranscript = transcript_data.map(function(line) {
                        return "<p data-start='" + line.start + "' data-duration='" + line.dur + "'>" 
                             + line.subtitle + "</p>";
                    }).join("");
                    ...
                    initializeTranscriptHighlighting(sha256);
                }
            }
        },
        error: function(xhr, status, error) {
            console.log("Transcript not found: " + transcript_url);
        }
    });
}

// Initialize transcript highlighting
function initializeTranscriptHighlighting(sha256) {
    var iframe = document.querySelector("#youtube_iframe_" + sha256);
    if (!iframe) return;
                ...
                'onStateChange': function(event) {
                    if (event.data == YT.PlayerState.PLAYING) {
                        setInterval(function() {
                            highlightCurrentTranscript(sha256);
                        }, 100);
                    }
                }
            }
...
function highlightCurrentTranscript(sha256) {
    var iframe = document.querySelector("#youtube_iframe_" + sha256);
    if (!iframe) return;
    ...
    jQuery("#transcript_" + sha256 + " p").each(function() {
        var start = parseFloat(jQuery(this).data('start'));
        var duration = parseFloat(jQuery(this).data('duration'));
    ...
    jQuery(this).each(function() {
        ...
        if (jQuery(this).attr("cite")) {
            ...
            if (cited_url.length > 3) {
				...
				console.log("============== Adding Quote to DOM ===============");
                ...
                jQuery.ajax({
                    ...
                    success: function(json) {
                        ...
                        // Load transcript if it's a YouTube video
                        var video_id = youtube_id(cited_url);
                        if (video_id) {
                            loadTranscriptData(video_id, json.sha256);
                        }
						else {
							console.log("cited_id is not video: " + cited_url);
						}
						

						console.log(cited_url);
						
						//console.log("============== Adding 2  Quote to DOM ===============");
						// console.log(json.cited_data);
                        ...
                        // alert("LOOKUP JSON: " + cited_url);

                        // alert(json.cited_transcript_data);
                ...
                function addQuoteToDom(tag_type, json, cited_url) {
                    ...
                    if (tag_type === "q") {
						...
						if(json.cited_url){
							...
							var url_cited_domain = '';
						}

                        //Add content to a hidden div, so that the popup can  later grab it
                        jQuery("#" + hidden_container).append(
                            "<div id='" + q_id + "' class='highslide-maincontent transcript width_" + popup_width + "'>" + 
                            embed_ui.html + " " + 
                            start_playing_at + 
                            "!-- <div id='transcript_before'>******** xx BEFORE *********</div -->" + 
                            "<div class='transcript_contents'>xxxxxxxx" + 
                            json.cited_context_before + " " + " <span class='q-tag-highlight'><strong>" +
                            json.citing_quote + "</strong></span> " +
                            json.cited_context_after + ".. </p>" +
                            "</div>" + 
                            "<!--div id='transcript_after'>******* xx AFTER *********</div></div-->" + 
							"<div id='transcript_" + json.sha256 + "' class='transcript'></div>" +
                            "<div class='popup-footer'>" +
                            "<p><a href='" + json.cited_url +
                            "' target='_blank'>Read more</a> | " +
                            "<a href='javascript:closePopup(`" +
                            json.sha256 + "`);'>Close</a> <div class='source_url'>source: <a href='" + json.cited_url + "'>" + url_cited_domain + "</a> </p></div>"
                        );
                        ...
                        if (media_type == 'video'){
							if (json.cited_url == cited_url){
								if (!(json.sha256 == undefined)){								
                        ...
                        if (media_type == 'text'){
							if ((json.cited_url == cited_url)  ){
								if (!(json.sha256 == undefined)){
									...
									" onclick='return expandPopup(this ,\"" + q_id + "\")' ><img src='https://www.citeit.net/assets/images/text-icon-small.png' " + 
									" class='text-icon' width='27' height='27' alt='text context' title='View Context: Text (no video)' /></a>";
 
									jQuery(json.sha256).append(text_icon);
								}
							 }
						 }
...
jQuery.fn.quoteContext2 = jQuery.fn.quoteContext;  // Alias
...
function youtube_id(url) {
    ...
    // Convert YouTube Livestream format
    url = url.replace("live/", "watch?v=");
...
function addTranscriptData(transcriptData, sha256, initialData) {
    ...
    if (transcriptContainer.length) {
        fullTranscript = transcriptData.map(function(line) {
        ...
        jQuery('#transcript_before').html('---Before:' + precedingTranscript + 'zzzzz');
        jQuery('#transcript_after').html('----After:' + remainingTranscript + 'eeeeeee');

		alert("================== PRECEDING TRANSCRIPT =================");
		// alert(precedingTranscript);
		console.log("================== PRECEDING TRANSCRIPT =================");
